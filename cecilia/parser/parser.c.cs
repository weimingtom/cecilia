...
namespace cecilia 
{
	public partical class cecilia 
	{
		...
		private static int
		shift(stack s, int type, CharPtr str, int newstate, int lineno)
		{
			int err;
			assert(!s_empty(s));
		...
		private static int
		classify(parser_state ps, int type, CharPtr str)
		{
		...
		public static int
		PyParser_AddToken(parser_state ps, int type, CharPtr str,
					  int lineno, IntPtr expected_ret)
		{
			register int ilabel;
			int err;
